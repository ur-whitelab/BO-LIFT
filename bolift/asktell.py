...
class AskTellFewShotMulti(AskTellFewShot):
    ...
    def _setup_inverse_prompt(self, example: Dict):
        ...
        if self._selector_k is not None:
            ...
            sim_selector = SemanticSimilarityExampleSelector if self.cos_sim else MaxMarginalRelevanceExampleSelector #LabelSimilarityExampleSelector #SemanticSimilarityExampleSelector if self.cos_sim else MaxMarginalRelevanceExampleSelector
            example_selector = sim_selector.from_examples(
                [example],
                OpenAIEmbeddings(),
                FAISS,
    ...
    def predict(self, x: str, system_message: Optional[str] = "") -> Union[Tuple[float, float], List[Tuple[float, float]]]:
        ...
        # compute mean and standard deviation
        if len(x) == 1:
            return (results[0], self._calibration_factor) if self._calibration_factor else results[0]

        # Ensure self._calibration_factor is set to 1 if it does not exist
    ...
    def _ask(
        ...
        aq_vals = [aq_fxn(r, best) for r in results]
        selected = np.argsort(aq_vals)[::-1][:k]
        # compare the output of the predict to aq_vals : -> print taht possible_x
        means = [r.mean() for r in results]
